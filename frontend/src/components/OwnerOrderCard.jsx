    ...
    const handleUpdateStatus = async (orderId, shopId, status) => {
        try {
            ...
            dispatch(updateOrderStatus({
                orderId,
                shopId,
                shopOrderId: result.data.shopOrder?._id,
                status,
    ...
    return (
        <div className='bg-white rounded-lg shadow p-4 space-y-4'>
            ...
            <div className='flex justify-between items-center mt-auto pt-3 border-t border-gray-100'>
                ...
                <select className='rounded-md border px-3 py-1 text-sm focus:outline-none focus:ring-2 border-[#ff4d2d] text-[#ff4d2d]' onChange={(e) => handleUpdateStatus(data._id, data.shopOrders.shop._id, e.target.value)} disabled={["cancelled", "delivered"].includes(data.shopOrders.status)}>
            ...
            <p className='text-xs text-gray-500'>
                Estimated delivery:
                <span className='font-semibold text-gray-700'> {data.shopOrders.estimatedDeliveryTime ? formatEstimatedTime(data.shopOrders.estimatedDeliveryTime) : (data.shopOrders.status === "cancelled" ? " Cancelled" : data.shopOrders.status === "delivered" ? " Completed" : " Updating")}</span>
            </p>

            {["out of delivery", "picked up"].includes(data.shopOrders.status) &&
                <div className="mt-3 p-2 border rounded-lg text-sm bg-orange-50 gap-4">
                    ...
                    {availableBoys?.length > 0 ? (
                        availableBoys.map((boy) => (
                            <div className='text-gray-800' key={boy.id}>{boy.fullName}-{boy.mobile}</div>
                        ))
